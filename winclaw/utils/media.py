...
def sniff_media_from_magic(data: bytes) -> FileType | None:
    ...
    if header.startswith(b"RIFF") and len(header) >= 12:
        ...
        if chunk == b"WAVE":
            return FileType(kind="audio", mime_type="audio/wav")
    if header.startswith(b"ID3") or (
        len(header) >= 2 and header[0] == 0xFF and (header[1] & 0xE0) == 0xE0
    ):
        return FileType(kind="audio", mime_type="audio/mpeg")
    if header.startswith(b"fLaC"):
        return FileType(kind="audio", mime_type="audio/flac")
    if header.startswith(b"OggS"):
        lowered = header.lower()
        if b"opushead" in lowered:
            return FileType(kind="audio", mime_type="audio/opus")
        return FileType(kind="audio", mime_type="audio/ogg")
    ...
    if brand := _sniff_ftyp_brand(header):
        ...
        if brand in _FTYP_AUDIO_BRANDS:
            return FileType(kind="audio", mime_type=_FTYP_AUDIO_BRANDS[brand])
    return None
...
def audio_format_from_mime(mime_type: str | None) -> str | None:
    if not mime_type:
        return None
    normalized = mime_type.split(";", 1)[0].strip().lower()
    return _AUDIO_FORMAT_BY_MIME.get(normalized)
