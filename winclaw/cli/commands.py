...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} winclaw[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} winclaw v{__version__}")
        raise typer.Exit()
...
@app.command()
def onboard():
    """Initialize winclaw configuration and workspace."""
...
def gateway(
    ...
    from winclaw.session.manager import SessionManager
...
def agent(
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]winclaw is thinking...[/dim]", spinner="dots")
    ...
    if message:
        ...
        from winclaw.bus.events import InboundMessage
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".winclaw" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # winclaw/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall winclaw")
        raise typer.Exit(1)
...
def status():
    ...
    console.print(f"{__logo__} winclaw Status\n")
