"""
agent3: REPL + Anthropic tools + workspace tools + MCP (examples/mcp.json).

Startup (async_main, one event loop)
    mcp.json  -->  load_mcp_tools_async  -->  connect each server + list_tools
                        |
                        +--> MCP_TOOLS_CACHE  +  _BASE_TOOLS  -->  TOOLS / TOOL_BY_NAME
    (_BASE_TOOLS: bash, read_file, write_file, edit_file)

REPL
    stdin  -->  history  -->  agent_loop(history)  -->  print assistant text

agent_loop (inner while)
...
Shutdown: cleanup_mcp_connections()


uv run python examples/agent3.py
"""
...
SYSTEM = (
    ...
    "MCP tools from configured servers are prefixed SERVER_NAME__ (see tool list). Act, don't explain."
)
...
TOOLS = [t.to_schema() for t in _BASE_TOOLS]
TOOL_BY_NAME: dict[str, Tool] = {t.name: t for t in _BASE_TOOLS}
...
def _register_all_tools(mcp_tools: list[Tool]) -> None:
    ...
    global TOOLS, TOOL_BY_NAME, MCP_TOOLS_CACHE
    MCP_TOOLS_CACHE = mcp_tools
    merged = _BASE_TOOLS + MCP_TOOLS_CACHE
    TOOLS = [t.to_schema() for t in merged]
    TOOL_BY_NAME = {t.name: t for t in merged}
...
async def async_main() -> None:
    ...
    try:
        while True:
            try:
                query = input("\033[36magent3 >> \033[0m")
