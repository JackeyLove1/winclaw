"""
agent15: agent14 feature stack + persistent Task System (DAG on disk under .tasks/) + Tool Validation + Cron


Run
    uv run python examples/agent15.py
"""
...
from examples.interactive import init_prompt_session, prompt_async
...
from examples.validator import clear_tool_input_validator_cache, validate_tool_input
...
CRON_TOOL = CronTool(WORKDIR)
PROMPT_HISTORY_FILE = WORKDIR / ".claude" / "history" / "agent15_history"
...
async def _execute_tool_block(
    ...
    validation_err = validate_tool_input(agent_tool, inp)
    if validation_err:
        return validation_err, extra_results

    decision = PERMISSION_MANAGER.check(block.name, inp)
    if decision["behavior"] == "deny":
        return f"Error: {decision['reason']}", extra_results
    if decision["behavior"] == "ask":
        if not await PERMISSION_MANAGER.ask_user_async(block.name, inp):
...
def _install_loop_exception_filter() -> None:
    """
    Suppress known noisy MCP shutdown races from transport internals.

    These errors can appear during cancellation/timeout while anyio task groups
    and async generators are closing, but do not affect agent state.
    """
    ...
    def handler(loop_obj: asyncio.AbstractEventLoop, context: dict[str, Any]) -> None:
        ...
        detail = f"{msg} {exc!s}"
        if any(token in detail for token in noisy_tokens):
            return
...
async def agent_loop(messages: list) -> None:
    max_output_recovery_count = 0
    while True:
        notifications = CRON_TOOL.drain_notifications()
        for note in notifications:
            print(f"[Cron notification] {note[:120]}")
...
async def _init_mcp_tools() -> list[Tool]:
    ...
    try:
        ...
        print(f"[WARN] MCP load cancelled, skip MCP tools: {e}")
        return []
    except Exception as e:
        print(f"[WARN] MCP load failed, skip MCP tools: {e}")
        return []
...
def _register_all_tools(mcp_tools: list[Tool]) -> None:
    ...
    clear_tool_input_validator_cache()
...
async def async_main() -> None:
    ...
    init_prompt_session(PROMPT_HISTORY_FILE)
    cron_started = False
    try:
        mcp_tools: list[Tool] = []
        try:
            ...
            # Final guard for cancellation-style MCP failures.
            print(f"[WARN] MCP bootstrap cancelled, continue without MCP: {e}")
            mcp_tools = []
        except Exception as e:
            # Final guard: even unexpected MCP bootstrap failures should not
            # block the agent from starting with built-in tools only.
            print(f"[WARN] MCP bootstrap failed, continue without MCP: {e}")
            mcp_tools = []
        _register_all_tools(mcp_tools)
        await CRON_TOOL.start()
        cron_started = True
